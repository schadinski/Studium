...
public class CheckingAccountTest {
	...
	private Class<CheckingAccount> bc;
	private List<String> errors;
	
	@Before
	public void setUp() {
		
		bc = CheckingAccount.class;
		errors = new ArrayList<String>();
	}
	
	@Test
	public void testCheckingAccountExtendsAccount() {
		List<String> errors = new ArrayList<String>();
		
		if(!bc.getSuperclass().getSimpleName().equalsIgnoreCase("Account")) {
			errors.add("Klasse CheckingAccount erweitert nicht die Klasse Account");	
		}
	...
	public void testConstructor() {
		try {
			Constructor<CheckingAccount> m = bc.getDeclaredConstructor(IPerson.class,Date.class, float.class);
			if((m.getModifiers() & Modifier.PUBLIC)==0)errors.add("Constructor ist nicht public");
		}catch(NoSuchMethodException exc) {
			errors.add("Constructor mit den Parametertypen IPerson, Calendar, float existiert nicht");
		}
		finally{
			assertTrue(errors.toString(), errors.size()==0);
		}
